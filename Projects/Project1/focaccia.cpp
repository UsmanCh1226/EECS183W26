...
int main() {
    ...
    int loaves = static_cast<int>(ceil(people / SERVINGS_PER_LOAF));
    
    int flourBags = static_cast<int>(ceil((loaves * CUPS_FLOUR_PER_LOAF) / 
                                     CUPS_FLOUR_PER_BAG));
    int yeastPackages = static_cast<int>(ceil((loaves * TSP_YEAST_PER_LOAF) / 
                                         TSP_YEAST_PER_PACKAGE));
    int saltCanisters = static_cast<int>(ceil((loaves * TSP_SALT_PER_LOAF) / 
                                         TSP_SALT_PER_CANISTER));
    
    double tbspPerBottle = ML_OIL_PER_BOTTLE / ML_PER_TBSP;
    int oilBottles = static_cast<int>(ceil((loaves * TBSP_OIL_PER_LOAF) / 
                                      tbspPerBottle));
